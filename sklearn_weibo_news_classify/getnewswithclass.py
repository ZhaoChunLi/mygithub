...
def addurl(url,visitedurl,unvisitedurl):
    if url and (url not in visitedurl) and (url not in unvisitedurl):
...
def geturls_thetitle(url,visitedurl):
    ...
    try:
        ...
        if rtitle:
            ...
            path_of_class=(r'f:/新闻/'+news_class).decode('utf-8')
            if not os.path.isdir(path_of_class):
                os.makedirs(path_of_class)
            path=(path_of_class+'/'+news_class+'_'+puretitle+'.txt').decode('utf-8')
...
def get_homepage(url,visitedurl,unvisitedurl):
    ...
    for link in links:
        if "people.com.cn/" in link and "/n1/"in link and "www" not in link and "tv" not in link:
...
class url_thread(Thread):
    def run(self):
        while self.setDaemon==True:
            visitedurl=[]
            unvisitedurl=[]
